...
if (isset($_GET['PayerID'])) {
    ...
    if ($paymentInfo === false) {
        ...
        try {
            ...
            $credits = $database->query("SELECT credits FROM ".TBL_INFO." WHERE uid = :uid", array(':uid' => $_GET['uid']))->fetchObject()->credits;
            $credits = $credits + $_SESSION['amount'];
            $items = array(':credits' => $credits, ':uid' => $_GET['uid']);
