...
@section('content')
    <form action="{{ route('shop.update', $item) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group{{ $errors->has('power') ? ' has-error' : '' }}">
            ...
            <input type="number" class="form-control" id="power" name="power" value="{{ old('power', $item->power) }}"
                   min="1">
            @if ($errors->has('power'))
                <p class="help-block">
        ...
        <div class="form-group{{ $errors->has('min_strength_points') ? ' has-error' : '' }}">
            ...
            <input type="number" class="form-control" id="min_strength_points" name="min_strength_points"
                   value="{{ old('min_strength_points', $item->min_strength_points) }}" min="0">
        ...
        <div class="form-group{{ $errors->has('max_amount') ? ' has-error' : '' }}">
            ...
            <input type="number" class="form-control" id="max_amount" name="max_amount"
                   value="{{ old('max_amount', $item->max_amount) }}" min="0">
        ...
        <a href="{{ route('shop.show', $item) }}" class="btn btn-warning">Cancel</a>
@endsection
